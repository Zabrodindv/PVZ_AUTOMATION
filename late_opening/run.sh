#!/bin/bash
#
# Запуск отчета по открытию ПВЗ с проверкой VPN
# Для использования с launchd/cron
#

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
LOG_FILE="$HOME/.late_opening_report.log"
...
check_vpn() {
    local hosts=("wms-clickhouse.prod.um.internal" "dwh-clickhouse.prod.um.internal")
...
# Основной скрипт
main() {
    log "=== Запуск отчета по открытию ПВЗ ==="
